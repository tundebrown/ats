...
async function dpopProofJwt(headers, options, url, htm, clockSkew, accessToken) {
    ...
    const proof = await jwt({
        ...
        ath: accessToken ? b64u(await crypto.subtle.digest({
            name: "SHA-256"
    ...
    switch(key.algorithm.name){
        case "ECDSA":
            return {
                ...
                hash: {
                    name: ecdsaHashName(key.algorithm.namedCurve)
                }
            };
        case "RSA-PSS":
            {
...
;// CONCATENATED MODULE: ./app/authconfig.js
const authConfig = {
    providers: [],
    pages: {
        signIn: "/login"
